...
/*   Updated: 2025/05/20 10:54:14 by qbarron          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_pixel_wall(t_raycast *raycast, t_game *game_st, t_texture *tex, int x)
{
	while (tex->y < raycast->draw_end)
	{
		...
		tex->d = tex->y * 256 - game_st->mlx_struct.win_h * 128 + raycast->line_h * 128;
		...
		if (tex->tex_x < 0 || tex->tex_x >= tex->width || tex->tex_y < 0 || tex->tex_y >= tex->height)
		{
		    tex->y++;
		    continue;
		}
		tex->color = *(int *)(tex->addr + (tex->tex_y * tex->line_len + tex->tex_x * (tex->bpp / 8)));
		if (raycast->side == 1)
...
void	init_texture_direction(t_raycast *raycast, t_game *game_st, t_texture *tex, int x)
{
	if (raycast->side == 0 && raycast->ray_dir_x < 0)
		...
		tex = &game_st->so;
	if (raycast->side == 0)
		tex->wall_x = game_st->player.y + raycast->wall_dist * raycast->ray_dir_y;
	else
		tex->wall_x = game_st->player.x + raycast->wall_dist * raycast->ray_dir_x;
